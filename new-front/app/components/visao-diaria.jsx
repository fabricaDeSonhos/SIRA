...
export default function VisaoDiaria({ dia, manhã, tarde, noite, noHours, noText }) {
  ...
  // CORREÇÃO DEFINITIVA: Garante que a data do dia atual esteja sempre no formato YYYY-MM-DD.
  // Isso é crucial para uma comparação de datas segura.
  ...
  // CORREÇÃO DEFINITIVA: A lógica de filtro foi alterada.
  // Em vez de usar `mesmo_dia`, que é suscetível a erros de fuso horário,
  // agora comparamos as strings de data no formato 'YYYY-MM-DD'. Isso é 100% confiável.
  const reservas = isLoading ? [] : reservations.filter(r => {
      ...
      return (
        <Reserva
          ...
          curso={r.curso}
          vazia={false}
          // ALTERAÇÃO: Garante que todas as reservas sejam editáveis.
          editavel={true}
          noText={noText}
        />
      );
